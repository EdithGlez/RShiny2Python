...
ui.input_slider("duration", "Minimum duration (min)", min=min(data["Duration"]), 
    max = max(data["Duration"]), value = min(data["Duration"]))
...
def _():
    df = data
    ui.update_slider("duration",  min = min(df["Duration"]), max = max(df["Duration"]),
    value=min(df["Duration"]))

# Filtered table
@render.data_frame
def table():
    return data[(data["Duration"] >= input.duration()) &
